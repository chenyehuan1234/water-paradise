...
export function removeLevel(p: ProjectData, id: string) { const chapter = p.chapters.find(c => c.levels.includes(id)); if (!chapter) return; const index = chapter.levels.indexOf(id); p.trash.push({ id: uid('trash'), name: p.levels[id].name, levels: [clone(p.levels[id])], chapterId: chapter.id, index }); chapter.levels.splice(index, 1); delete p.levels[id]; if (p.active === id) p.active = p.chapters.flatMap(c => c.levels)[0] ?? ''; }
...
export function importProject(p: ProjectData, text: string): ProjectData {
  ...
  if (raw && typeof raw === 'object' && 'chapters' in raw) {
    ...
    for (const c of parsed) { const chapter = addChapter(next, c.name); for (const level of c.levels) { level.id = uid('import'); addLevel(next, chapter, level); } }
