...
app.innerHTML=`<main class="app">
  ...
  <div class="canvas-tools"><button id="toggle-left" aria-label="收起或展开关卡栏">☰ 关卡</button><button id="toggle-right" aria-label="收起或展开物件库">▦ 物件</button><label class="motion-select">动作<select id="animation-mode" aria-label="动作动画"><option value="full">完整</option><option value="simple">简化</option><option value="system">跟随系统</option></select></label><button id="pause">暂停水面</button><button id="restart" hidden>重开</button><span class="tool-status" id="tool-status"></span></div><div class="stats-chip"><div id="water-stats"></div><div id="render-stats">准备画面</div></div><div class="win-card" id="win" hidden><span class="eyebrow">COURTYARD COMPLETE</span><h2>水，找到了归途。</h2><p>唯一出口已抵达 · 庭院完成</p><div class="win-actions"><button class="primary" id="next-level">下一关 ↗ <kbd>Enter</kbd></button><button id="replay-level">再玩一遍</button></div></div><div class="error-banner" id="render-error" role="alert" hidden></div><div class="loading" id="loading">正在唤醒庭院…</div></section>
...
function refreshStatus(){
  ...
  $<HTMLButtonElement>('next-level').innerHTML=nextPlayable()?'下一关 ↗ <kbd>Enter</kbd>':indexPlay?'打开选关 ↗ <kbd>Enter</kbd>':'返回编辑 ↗ <kbd>Enter</kbd>';
}
...
async function navigateHome(){clearMovement();clearTimeout(saveTimer);if(await save())location.href='./';}
...
function act(command:Command,heldKey?:string):boolean{if(!game)return false;if(view?.busy){if(heldKey)queuedKey=heldKey;else pending=command;return false;}const result=game.act(command);if(!result.accepted){if(heldKey)blockedHeld=heldKey;else notice(result.message);return false;}blockedHeld=null;water=result.water;view?.show(game.level,game.state.objects,water,editor.settings,game.state,result.events);refreshStatus();if(indexPlay){if(result.state.status==='won'){clearLevelProgress(game.level.id);if(listedInGame(project,game.level.id)&&!completed.has(game.level.id)){completed.add(game.level.id);renderNavigation();void store.markCompleted(game.level.id).catch(()=>notice('通关已完成，但本地进度暂时无法保存。'));}}else{saveLevelProgress(game.level.id,game.state);}}return true;}
function moveByKey(k:string){if(!view||!game)return;if(view.busy){queuedKey=k;return;}const direction=view.movementDirection(moveKeys[k]);
...
function startHeld(k:string){if(!game||held.has(k))return;held.add(k);heldLast=k;if(view?.busy)queuedKey=k;else moveByKey(k);}
...
function togglePlay(){finishStroke();clearMovement();setLevelDrawer(false);view?.finish(false);if(game){game=null;app.classList.toggle('left-closed',leftBeforePlay);history.replaceState(null,'',`./?level=${encodeURIComponent(project.active)}`);notice('已返回编辑，试玩中的位置没有覆盖草稿。');}else{try{game=new GameSession(editor.level);const restored=indexPlay&&restoreLevelProgress(game);if(restored)notice('已恢复上次进度');}catch(error){notice(`暂不能试玩：${(error as Error).message}`);return;}leftBeforePlay=app.classList.contains('left-closed');app.classList.add('left-closed');history.replaceState(null,'',`./?mode=play&level=${encodeURIComponent(project.active)}`);}syncControls();renderNavigation();draw();$('world').focus();}
function leaveIndexPlay(){void navigateHome();}
$('play').onclick=()=>indexPlay&&game?leaveIndexPlay():togglePlay();const restartGame=()=>{clearMovement();view?.finish(false);game?.restart();if(indexPlay&&game)clearLevelProgress(game.level.id);draw();};$('restart').onclick=restartGame;$('mobile-restart').onclick=restartGame;
function nextLevel(){if(!game||game.state.status!=='won'||$('win').hidden)return;const next=nextPlayable();if(!next){if(indexPlay)setLevelDrawer(true);else togglePlay();return;}const restore=leftBeforePlay;selectLevel(next);togglePlay();leftBeforePlay=restore;}
$('next-level').onclick=nextLevel;$('replay-level').onclick=()=>$('restart').click();
function undo(){clearMovement();finishStroke();if(game){const result=game.undo();if(result){water=result.water;view?.show(game.level,game.state.objects,water,editor.settings,game.state,result.events);refreshStatus();if(indexPlay&&game.state.status!=='won')saveLevelProgress(game.level.id,game.state);}}else if(editor.undo()){scheduleSave();capture();renderNavigation();draw(true);}}
function redo(){clearMovement();if(game){const result=game.redo();if(result){water=result.water;view?.show(game.level,game.state.objects,water,editor.settings,game.state,result.events);refreshStatus();if(indexPlay&&game.state.status!=='won')saveLevelProgress(game.level.id,game.state);}}else if(editor.redo()){scheduleSave();capture();renderNavigation();draw(true);}}
...
canvas.addEventListener('pointerdown',e=>{if(game||(e.button!==0&&e.button!==2)||!view)return;e.preventDefault();canvas.focus();const erase=e.button===2,pick=view.pick(e.clientX,e.clientY);if(!pick)return;editor.begin(erase,pick);strokeButton=e.button;canvas.setPointerCapture(e.pointerId);rectangleEnd=erase?editor.erasePosition(pick):resolvePlacement(pick,editor.settings).position;if(!effectiveRectangle()){editor.paint(pick);requestDraw();}});
...
window.addEventListener('keydown',e=>{if((e.target as HTMLElement).closest('input,select,textarea,dialog'))return;const k=e.key.toLowerCase();if((e.ctrlKey||e.metaKey)&&k==='z'){e.preventDefault();e.shiftKey?redo():undo();return;}if((e.ctrlKey||e.metaKey)&&k==='y'){e.preventDefault();redo();return;}if((e.ctrlKey||e.metaKey)&&k==='s'){e.preventDefault();void save();return;}
  if(game){if(app.classList.contains('level-drawer-open')){if(k==='escape'){e.preventDefault();setLevelDrawer(false);}return;}if(k==='enter'&&!$('win').hidden){e.preventDefault();nextLevel();return;}if(moveKeys[k]){e.preventDefault();if(!e.repeat)startHeld(k);}else if(k===' '){e.preventDefault();if(!e.repeat)act({type:'pull'});}else if(k==='z')undo();else if(k==='y')redo();else if(k==='r')$('restart').click();else if(k==='escape')indexPlay?leaveIndexPlay():togglePlay();}
