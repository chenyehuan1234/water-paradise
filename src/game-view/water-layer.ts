...
import type { LevelDataV1, WaterEdge, WaterSolution } from '../core/types';
...
const surfaceFragment = `
  ...
  void main() {
    ...
    gl_FragColor=vec4(color,vAlpha*(vDepth>.01?.87:.66));
...
export function skyHeight(water: WaterSolution) { return Math.max(0, ...water.cells.flat().filter(c => c !== null).map(c => c!.ground)) + 4; }
export const worldCell = (level: LevelDataV1, x: number, z: number): [number, number] => [x - (level.width - 1) / 2, z - (level.depth - 1) / 2];
...
export class WaterLayer {
  ...
  private surfaceMaterial = new ShaderMaterial({
    uniforms: { time: { value: 0 }, progress: { value: 1 }, showDepth: { value: 0 }, deepColor: { value: new Color('#21798f') }, lightColor: { value: new Color('#68c5c8') }, foamColor: { value: new Color('#f2f6e5') } },
  ...
  setSolution(level: LevelDataV1, water: WaterSolution, animate: boolean) {
    const next = new Map<string, SurfaceTransition>();
    for (const row of water.cells) for (const cell of row) {
      if (!cell || cell.kind === 'dry') continue;
      const key = `${cell.x},${cell.z}`, old = this.surfaces.get(key), [x,z] = worldCell(level, cell.x, cell.z);
      const edge = (dx: number, dz: number) => { const neighbor = water.cells[cell.z+dz]?.[cell.x+dx]; return neighbor && neighbor.kind !== 'dry' && neighbor.level === cell.level ? 0 : 1; };
      next.set(key, { x, z, fromY: old ? old.fromY+(old.toY-old.fromY)*this.progress : cell.ground+.025, toY: cell.level+.025, fromAlpha: old ? old.fromAlpha+(old.toAlpha-old.fromAlpha)*this.progress : 0, toAlpha: 1, depth: cell.depth, edges: [edge(-1,0),edge(1,0),edge(0,-1),edge(0,1)] });
    }
    ...
    if (level.source) {
      const cell = water.cells[level.source.z]?.[level.source.x];
      if (cell) {
        const [x,z] = worldCell(level,cell.x,cell.z), top=skyHeight(water), bottom=cell.level+.03, w=.13;
  ...
  update(delta:number,paused:boolean){if(!paused)this.elapsed+=delta;this.progress=Math.min(1,this.progress+delta/.3);for(const material of [this.surfaceMaterial,this.fallMaterial]){material.uniforms.time.value=this.elapsed;material.uniforms.progress.value=this.progress;}}
