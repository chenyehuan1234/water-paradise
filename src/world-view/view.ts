import { AmbientLight, Box3, BufferGeometry, Color, DirectionalLight, DoubleSide, FogExp2, GridHelper, Group, HemisphereLight, InstancedMesh, Line, LineBasicMaterial, Matrix4, Mesh, MeshBasicMaterial, Object3D, OrthographicCamera, PCFShadowMap, Plane, Raycaster, Scene, SphereGeometry, SRGBColorSpace, Vector2, Vector3, WebGLRenderer, ACESFilmicToneMapping, BoxGeometry, TorusGeometry } from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { MOUSE } from 'three';
import { dynamic, glass, top } from '../world/catalog';
...
import { gargoyle, materials, prepareMaterials, prototype } from './models';
...
export class WorldView {
  ...
  private chain = new Line(new BufferGeometry(),new LineBasicMaterial({color:'#e8d7ad'}));
  private chainLinks = new InstancedMesh(new TorusGeometry(.028,.009,4,8),materials.silver,420);
  ...
  reset(){if(!this.level)return;const h=Math.max(2,...this.objects.map(o=>top(o)));this.span=Math.max(9,Math.max(this.level.width,this.level.depth)*1.12+h*.55);this.controls.target.set(0,h*.4,0);this.camera.zoom=1;this.camera.position.copy(this.controls.target).add(new Vector3(14,25,21));this.updateProjection();this.controls.update();}
  ...
  get presentation(){const positions:Record<string,number[]>={player:this.avatar.root.position.toArray()};for(const [id,g]of this.dynamicGroups)positions[id]=g.position.toArray();return {positions,water:this.water.snapshot,progress:this.motion?this.motion.elapsed/this.motion.duration:1};}
  ...
  private proto(kind:WorldObject['kind']){if(!this.prototypes.has(kind))this.prototypes.set(kind,prototype(kind));return this.prototypes.get(kind)!;}
  private clearStatic(){for(const child of [...this.staticGroup.children]){this.staticGroup.remove(child);if(child instanceof InstancedMesh)child.dispose();}}
  show(level:LevelDataV2,objects:WorldObject[],solution:WaterSolution,settings:EditorSettings,state?:GameState,events:ActionEvent[]=[],editorTransition=false){
    ...
    const staticKey=JSON.stringify([level.id,settings.slice,objects.filter(o=>!dynamic(o))]),rebuildStatic=staticKey!==this.staticSignature;this.staticSignature=staticKey;if(rebuildStatic)this.clearStatic();
    ...
    const visible=displayObjects.filter(o=>o.y<=settings.slice), buckets=new Map<WorldObject['kind'],WorldObject[]>();
    ...
    for(const o of visible){if(enclosed(o))continue;if(dynamic(o)){let g=this.dynamicGroups.get(o.id);if(!g){g=this.proto(o.kind).clone(true);this.movingGroup.add(g);this.dynamicGroups.set(o.id,g);}this.positionObject(g,o);}else if(rebuildStatic){if(!buckets.has(o.kind))buckets.set(o.kind,[]);buckets.get(o.kind)!.push(o);}}
    ...
    for(const [kind,list] of buckets){const proto=this.proto(kind);for(const part of proto.children){if(!(part instanceof Mesh))continue;const mesh=new InstancedMesh(part.geometry,part.material,list.length);mesh.castShadow=part.castShadow;mesh.receiveShadow=true;mesh.renderOrder=part.renderOrder;
  ...
  private markerGeometry=new SphereGeometry(.1,8,6);private markerMaterial=new MeshBasicMaterial({color:'#f3d078'});private sourceMaterial=new MeshBasicMaterial({color:'#f0ffec'});
  private buildMarkers(water:WaterSolution){this.markers.clear();if(!this.level)return;if(this.settings?.outlets)for(const e of water.outlets){const d=dir(e.direction),m=new Mesh(this.markerGeometry,this.markerMaterial);this.positionObject(m,{x:e.x+d.x*.48,y:e.from+.11,z:e.z+d.z*.48});this.markers.add(m);}if(this.editor&&this.level.source){const m=new Mesh(this.markerGeometry,this.sourceMaterial);this.positionObject(m,{...this.level.source,y:(water.sourceLanding?.y??this.level.source.y)+.6});m.scale.setScalar(1.4);this.markers.add(m);}}
  ...
  pick(clientX:number,clientY:number,snapshot?:LevelDataV2):Pick|null{
    if(!this.level||!this.settings)return null;this.setRay(clientX,clientY);const objects=snapshot?.objects??this.objects;let closest=Infinity,pick:Pick|null=null;
    const test=(o:WorldObject|{id:string;kind:'spawn'|'source';x:number;y:number;z:number;direction:Direction})=>{
      ...
      if(o.kind==='source'||o.kind==='spawn'){sx=sz=.2;sy=.4;cy=o.y+.4;}else if(glass(o as WorldObject)){const d=dir(o.direction);cx+=d.x*.5;cz+=d.z*.5;if(d.x)sx=.11;else sz=.11;}else if(o.kind==='bridge'){sy=.035;cy=o.y-.02;}else if(o.kind==='barrier'){sy=1;cy=o.y+1;}else if(o.kind==='boat'){sy=.11;cy=o.y-.09;}else if(o.kind==='plant'||o.kind==='arch'){sx=sz=.3;}
      ...
      pick={id:o.id,kind:o.kind,position:{x:o.x,y,z:o.z},normal,direction:o.direction,hit:{x:p.x+(this.level!.width-1)/2,y:p.y,z:p.z+(this.level!.depth-1)/2}};
    };
    for(const o of objects)test(o);
    ...
    const hit={x:this.rayPoint.x+(this.level.width-1)/2,y:this.rayPoint.y,z:this.rayPoint.z+(this.level.depth-1)/2};return {position:p,normal:{x:0,y:1,z:0},hit};
  }
  ...
  preview(placement:Placement|null,pick:Pick|null,erase=false){this.ghost.visible=!!placement&&!erase;this.highlight.visible=!!pick?.id&&erase;if(placement){this.positionObject(this.ghost,placement.position);this.ghost.position.y+=.5;this.ghost.scale.set(1,1,1);this.ghost.material.color.set(placement.error?'#ce6e59':'#94d0b3');const tool=this.settings?.tool;if(tool==='glass'||tool==='open-glass'){const d=dir(placement.direction);this.ghost.position.x+=d.x*.5;this.ghost.position.z+=d.z*.5;this.ghost.scale.set(d.x?.06:1,1,d.z?.06:1);}if(tool==='bridge'){this.ghost.position.y-=.5;this.ghost.scale.y=.06;}if(tool==='barrier'){this.ghost.scale.y=2;this.ghost.position.y+=.5;}}if(pick){this.positionObject(this.highlight,pick.position);this.highlight.position.y+=.5;this.highlight.scale.set(1.025,1.025,1.025);if(pick.kind==='glass'||pick.kind==='open-glass'){const d=dir(pick.direction!);this.highlight.position.x+=d.x*.5;this.highlight.position.z+=d.z*.5;this.highlight.scale.set(d.x?.14:1.025,1.025,d.z?.14:1.025);}}}
  ...
  private animate(delta:number){const motion=this.motion;if(!motion||!this.level)return;motion.elapsed+=delta;const t=Math.min(1,motion.elapsed/motion.duration),smooth=t*t*(3-2*t),byId=new Map<string,ActionEvent[]>();
    for(const e of motion.events)if(!['land','splash','drip','win','turn'].includes(e.type)&&e.id!=='chain'){if(!byId.has(e.id))byId.set(e.id,[]);byId.get(e.id)!.push(e);}
    const category=(e:ActionEvent)=>e.type==='fall'||e.type==='leave'?1:e.type==='load'?2:e.type==='float'?3:e.type==='slide'?4:0;
    const reverse=motion.events.some(e=>e.reversed);const phases=[...new Set([...byId.values()].flat().map(category))].sort((a,b)=>reverse?b-a:a-b), phaseIndex=Math.min(phases.length-1,Math.floor(t*phases.length)), activePhase=phases[phaseIndex],phaseTime=Math.min(1,t*phases.length-phaseIndex);
    ...
    if(pull?.target&&this.chain.visible){const a=this.avatar.root.position.clone().add(new Vector3(0,.52,0)),[tx,tz]=worldCell(this.level,pull.target.x,pull.target.z),d=dir(this.state!.facing),target=pull.targetId?this.dynamicGroups.get(pull.targetId):null;const b=target?target.position.clone().add(new Vector3(0,.52,0)):new Vector3(tx,pull.target.y+.52,tz);b.x-=d.x*.46;b.z-=d.z*.46;
      const extend=Math.min(1,t/.13,(1-t)/.12),end=a.clone().lerp(b,extend),points:Vector3[]=[];const count=Math.min(420,Math.max(2,Math.ceil(a.distanceTo(end)/.042))),m=new Object3D();
  ...
  finish(notify=true){if(!this.motion)return;this.motion=null;this.water.finish();for(const o of this.objects.flatMap(o=>o.kind==='boat'&&o.cargo?.[0]?[o,{...o,...o.cargo[0]}]:[o])){const m=this.dynamicGroups.get(o.id);if(m)this.positionObject(m,o);}for(const [id,g]of this.dynamicGroups)if(!this.objects.some(o=>o.id===id||o.cargo?.some(c=>c.id===id))){this.movingGroup.remove(g);this.dynamicGroups.delete(id);}if(this.state)this.positionObject(this.avatar.root,{...this.state.player,direction:this.state.facing});this.avatar.body.position.y=0;this.avatar.body.rotation.x=0;this.avatar.body.scale.y=1;this.avatar.head.rotation.x=0;for(const f of this.avatar.feet)f.rotation.x=0;this.chain.visible=this.chainLinks.visible=this.spit.visible=false;if(notify)this.onIdle?.();}
  private fadeEditor(){this.canvas.animate([{opacity:.65},{opacity:1}],{duration:140,easing:'ease-out'});}
  get busy(){return !!this.motion;}
  private frame=()=>{this.raf=requestAnimationFrame(this.frame);const now=performance.now(),delta=Math.min(.05,(now-this.last)/1000);this.last=now;if(this.failed||document.hidden)return;if(!this.paused)this.elapsed+=delta;this.water.update(delta,this.paused);this.animate(delta);if(!this.motion&&!this.reduced)this.avatar.body.scale.y=1+Math.sin(this.elapsed*2.2)*.016;
    for(const r of [...this.rings]){r.time+=delta;r.mesh.scale.setScalar(1+r.time*2*r.strength);(r.mesh.material as MeshBasicMaterial).opacity=Math.max(0,.7-r.time);if(r.time>.75){this.animatedEffects.remove(r.mesh);r.mesh.geometry.dispose();(r.mesh.material as MeshBasicMaterial).dispose();this.rings.splice(this.rings.indexOf(r),1);}}
  ...
  dispose(){cancelAnimationFrame(this.raf);this.observer.disconnect();this.controls.dispose();this.water.dispose();this.clearStatic();this.renderer.dispose();}
}
