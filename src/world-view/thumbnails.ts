...
import { balanceModel, gargoyle, prototype } from './models';
...
export function fillThumbnails(root: HTMLElement) {
  ...
  for(const button of root.querySelectorAll<HTMLButtonElement>('[data-tool]')){
    ...
    const model=kind==='spawn'?gargoyle().root:kind==='balance'?balanceModel(2).root:prototype(kind as ObjectKind);if(kind==='barrier'){model.scale.setScalar(.6);model.position.y=-.02;}if(kind==='bridge')model.position.y=.5;
    if(kind==='balance'){model.scale.setScalar(.29);model.rotation.y=-.4;model.position.y=.05;}
